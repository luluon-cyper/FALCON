...
def prepare_file_with_placeholder(input_path: str, output_path: str, placeholder_len: int = 2048) -> None:
...
def extract_byte_range_and_placeholder(pdf_bytes: bytes, index: int = -1) -> Tuple[List[int], int, int]:
    containers = []
...
def embed_signature_into_pdf(pdf_bytes: bytes, signature: bytes, index: int = -1) -> bytes:
    _, maker_start, maker_end = extract_byte_range_and_placeholder(pdf_bytes, index)
...
def extract_signature_hex_from_pdf(pdf_bytes: bytes, index: int = -1) -> bytes:
    _, marker_start, marker_end = extract_byte_range_and_placeholder(pdf_bytes , index)
